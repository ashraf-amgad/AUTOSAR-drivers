		...
		if(error == FALSE)
		{
			...
			for(CAN_BaudRate_Counter = 0 ; CAN_BaudRate_Counter < MAX_NUMBER_OF_SUPORTED_BAUDRATE ; CAN_BaudRate_Counter++)
			{
				...
				if(CAN_BaudRate_Ptr[CAN_BaudRate_Counter].CanControllerBaudRate == CAN_CONTROLLER_DEFAULT_BAUDRATE)
				{
					...
					CAN1_BTR |= (uint32) ((uint32) ((Ph_Seg1-1) << 16)) |\
															 ((uint32) ((CAN_BaudRate_Ptr[CAN_BaudRate_Counter].CanControllerSeg2 -1) << 20)) |\
															 ((uint32) ((CAN_BaudRate_Ptr[CAN_BaudRate_Counter].CanControllerSyncJumpWidth -1) << 24)) |\
			...
			for(CAN_HOH_Counter = 0 ; CAN_HOH_Counter < MAX_NUMBER_OF_HARDWARE_OBJECT ; CAN_HOH_Counter++)
			{
					if(CAN_HOH_Ptr[CAN_HOH_Counter].CanObjectType == CanObjectType_RECEIVE)
					{
							...
							if(CAN_Filter_Ptr->CanFilterScale == CAN_FilterScale_16bit)
							{
								...
								CAN1->sFilterRegister[CAN_Filter_Ptr->CanFilterNumber].FR1 = (uint32)
								( ( ( (uint32) (0x0000FFFF) & (uint32)(CAN_Filter_Ptr->CanFilterReg2 & 0x0000FFFF) ) << 16 ) |
									( (uint32) (0x0000FFFF) & (uint32)(CAN_Filter_Ptr->CanFilterReg1 & 0x0000FFFF) ) );

								/* Second 16-bit identifier and Second 16-bit mask */
								/* Or Third 16-bit identifier and Fourth 16-bit identifier */
								CAN1->sFilterRegister[CAN_Filter_Ptr->CanFilterNumber].FR2 = (uint32)
								( ( (uint32) (0x0000FFFF) & (uint32)(CAN_Filter_Ptr->CanFilterReg2 >> 16 ) ) |
								  ( (uint32) (0x0000FFFF) & (uint32)(CAN_Filter_Ptr->CanFilterReg1 >> 16 ) ) );
							...
							else if (CAN_Filter_Ptr->CanFilterScale == CAN_FilterScale_32bit)
							{
								...
								CAN1->sFilterRegister[CAN_Filter_Ptr->CanFilterNumber].FR1 = (uint32)
								((uint32)(0x7FF00000) & (uint32)(CAN_Filter_Ptr->CanFilterReg1 ) << 21);
								
								/* 32-bit mask or Second 32-bit identifier */
								CAN1->sFilterRegister[CAN_Filter_Ptr->CanFilterNumber].FR1 = (uint32)
								((uint32)(0x7FF00000) & (uint32)(CAN_Filter_Ptr->CanFilterReg2 ) << 21);
							}
							else 
							{
								/* No action needed */
...
void Can_DisableControllerInterrupts( uint8 Controller )
{
	...
	//CANIntEnable(Global_Config->CanHardwareObjectRef[Controller].CanControllerRef->CanControllerBaseAddress,CAN_CTL_IE);
		...
		if(FALSE == error)
		{
			...
			{
				case (CAN_0_ID):
					CAN1_IER = CAN_DISABLE_INTERUPTS;
				break;
...
Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType* PduInfo )
{
	 uint8 Temp_PduInfo_DLC =0;
		uint8 transmit_mailbox=0;/*use another type idiot,islam says*/
  ...
  {
			/* Set up the Id */
			CAN1->sTxMailBox[transmit_mailbox].TIR &= TMIDR_TXRQ;
      ...
      CAN1->sTxMailBox[transmit_mailbox].TIR |= (PduInfo->id << 21);
			...
			CAN1->sTxMailBox[transmit_mailbox].TDTR &= (uint32)0xFFFFFFF0;
